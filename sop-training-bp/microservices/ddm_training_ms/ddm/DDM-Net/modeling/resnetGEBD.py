...
from .tao_pytorch_backbone_v2.dino_v2 import vit_large_patch14_dinov2_swiglu, vit_large_patch14_dinov2_swiglu_legacy, vit_giant_patch14_reg4_dinov2_swiglu
from .tao_pytorch_backbone_v2.segformer_backbones_dino_v2 import vit_large_nvdinov2, vit_giant_nvdinov2
from timm.layers import trunc_normal_
from .resnetGEBD_init_strategies import (
    init_weights_xavier, init_weights_kaiming,
    init_weights_scaled, init_weights_standard
)
...
model_urls = {
    "resnet50": "https://api.ngc.nvidia.com/v2/models/nvidia/resnet50_pyt_amp/versions/20.06.0/files/nvidia_resnet50_200821.pth.tar",
}
...
class resnetGEBD(nn.Module):
    def __init__(
        self, backbone="resnet50", pretrained=True, num_classes=2, frames_per_side=5, freeze_backbone=True, resolution=224
    ):
        ...
        if backbone.lower() == "resnet50":
            ...
            self.backbone = vit_large_nvdinov2(resolution=resolution, freeze_at=freeze_at)
            self.dinov2_feature_dims = [1024, 1024, 1024, 1024]
            if pretrained:
                ...
                print(f"{msg}")
                del state_dict
        elif backbone.lower() == "nvdinov2_giant":
            self.backbone = vit_giant_nvdinov2(resolution=resolution, freeze_at=freeze_at)
            self.dinov2_feature_dims = [1536, 1536, 1536, 1536]
            if pretrained:
                state_dict = torch.load(pretrained, map_location="cpu")
                msg = self.backbone.load_state_dict(state_dict, strict=False)
                print(f"Loaded pretrained weights from {pretrained}")
        ...
        if "nvdinov2" in backbone.lower():
            ...
            self.x3_out = nn.Sequential(
                nn.Conv1d(self.dinov2_feature_dims[2], 512, 1),
                nn.ReLU(inplace=True),
                nn.Conv1d(512, 512, 1),
            )
            self.x4_out = nn.Sequential(
                nn.Conv1d(self.dinov2_feature_dims[3], 512, 1),
                nn.ReLU(inplace=True),
                nn.Conv1d(512, 512, 1),
            )
        else:
        ...
        self.pos2 = PositionEmbeddingSine(num_locations=num_locations)

        # Initialize only non-backbone modules
        self._init_non_backbone_weights()

    def _init_non_backbone_weights(self):
        """Initialize weights for all modules except the backbone with enhanced strategies.

        This method applies different initialization strategies based on module types:
        - Transformer modules: Xavier initialization (optimal for attention mechanisms)
        - CNN modules: Kaiming initialization (optimal for ReLU activations)
        - Projection layers: Scaled initialization with reduced magnitude
        - Output layers: Small initialization for training stability
        - DINOv2 adaptation layers: Very small initialization to preserve pretrained features
        - Position embeddings: Standard initialization
        - Alpha parameters: Zero initialization for residual connections
        """

        # 1. Transformer modules - Xavier initialization (better for attention)
        transformer_modules = [
            self.intra_transformer1, self.intra_transformer2,
            self.cross_transformer1, self.cross_transformer2
        ]
        for module in transformer_modules:
            if module is not None:
                module.apply(init_weights_xavier)

        # 2. CNN modules - Kaiming initialization (better for ReLU)
        cnn_modules = [
            self.ddm_encoder, self.short_conv,
            self.middle_conv, self.long_conv, self.ddm_decoder
        ]
        for module in cnn_modules:
            if module is not None:
                module.apply(init_weights_kaiming)

        # 3. Projection layers - Scaled initialization
        projection_modules = [
            self.proj_rgb, self.proj_ddm, self.proj_v
        ]
        for module in projection_modules:
            if module is not None:
                module.apply(lambda m: init_weights_scaled(m, scale=0.5))

        # 4. Output layers - Smaller initialization for stability
        output_modules = [self.fc, self.fc2]
        ...
        # 5. DINOv2 adaptation layers - careful initialization
        if hasattr(self, 'x2_out'):
            self.x2_out.apply(lambda m: init_weights_scaled(m, scale=0.1))
        if hasattr(self, 'x3_out'):
            self.x3_out.apply(lambda m: init_weights_scaled(m, scale=0.1))
        if hasattr(self, 'x4_out'):
            self.x4_out.apply(lambda m: init_weights_scaled(m, scale=0.1))

        # 6. Position embeddings - standard initialization
        if hasattr(self, 'pos'):
            self.pos.apply(init_weights_standard)
        if hasattr(self, 'pos2'):
            self.pos2.apply(init_weights_standard)

        # 7. Alpha parameter - zero initialization for residual connections
    ...
    def forward(self, x):
        ...
        if "nvdinov2" in self.backbone_name:
            _, x2, x3, x4 = self.backbone.forward_feature_pyramid(x)
        else:
