...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --choice-file) CHOICES="$2"; shift ;;
        --video-root) VIDEO_ROOT="$2"; shift ;;
        --exclude-action) EXCLUDE_ACTIONS="$2"; shift ;;
        --video-ext) EXT="$2"; shift ;;
        --max-chunk-len) MAX_CHUNK_LEN="$2"; shift ;;
...
python autolabel_augmenting/config_to_sequential_mcq.py \
       --config-root "${CONFIG_ROOT:-}" \
       --action-json "${ACTION_JSON:-}" \
       --video-root $VIDEO_ROOT \
       --exclude-action $EXCLUDE_ACTIONS \
       --ext $EXT \
