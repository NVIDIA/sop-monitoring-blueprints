...
from utils.utils import create_file, dump_toml, parse_cr_log, read_toml, terminate_process_tree
...
app = FastAPI(
    title="Cosmos-Reason1 Fine-tuning Microservice",
    description="A microservice for fine-tuning Cosmos-Reason1 models",
    openapi_tags=[
        {"name": "fine-tuning", "description": "Operations for fine-tuning Cosmos-Reason1 models"},
...
async def root():
    ...
    return {"message": "Cosmos-Reason1 Fine-tuning Microservice is running"}
...
async def start_fine_tuning(dataset_id: str, background_tasks: BackgroundTasks):
    ...
    This endpoint initiates a new Cosmos-Reason1 fine-tuning job with the provided configuration.
...
async def run_fine_tuning(job_id: str, dataset_path: str, train_config_path: str, custom_dataset_path: str):
    ...
    try:
        ...
        logger.info(f"Started Cosmos-Reason1 process with PID {process.pid} for job {job_id}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start fine-tuning: {traceback.format_exc()}")
