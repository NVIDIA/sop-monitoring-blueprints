...
const DataAugmentationPanel = ({ allVideoResults, augmentedDatasets, onAugmentationComplete }) => {
  ...
  const handleStartAugmentation = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/augment?label_data_id=${selectedDatasetId}`, {
        method: 'POST',
        headers: {
                ...
                {selectedDatasetId && (
                  <div className="mb-3">
                    ...
                    <ul className="list-unstyled small text-muted">
                      ...
                      <li>✂️ Total Clips: {getDatasetStats(selectedDatasetId).totalClips}</li>
                      <li>⏱️ Total Duration: {getDatasetStats(selectedDatasetId).totalDuration.toFixed(2)}s</li>
                    </ul>
                  </div>
                )}
              </Col>
