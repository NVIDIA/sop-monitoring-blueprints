...
class TestVLMAugmentationServiceGqaToGqas:
    ...
    async def test_gqa_to_gqas_uses_env_var_fallback(self):
        ...
        augment_config = {
            "video_extention": "mp4",
            "gqas": {
                # No ngc_personal_key specified, should fall back to env var
                "llm_type": "nvidia",
            },
        }
        ...
        with patch.object(service, "_run_cmd", new_callable=AsyncMock) as mock_run:
            ...
            with patch.dict(os.environ, {"NGC_PERSONAL_KEY": "env-key"}):
                ...
                cmd = mock_run.call_args[0][0]
                api_key_idx = cmd.index("--api-key") + 1
                assert cmd[api_key_idx] == "env-key"
...
class TestVLMAugmentationServiceProcessAllActions:
    ...
    @pytest.mark.asyncio
    async def test_process_all_actions_no_video_folders_raises(self, tmp_path):
        """Test that process_all_actions raises when no video folders found."""
        from app import VLMAugmentationService
        from fastapi import HTTPException
        ...
        assert exc_info.value.status_code == 400
        assert "No video folders found" in exc_info.value.detail
    ...
    async def test_process_all_actions_handles_failure(self, tmp_path):
        """Test that process_all_actions handles stage failure properly."""
        from app import VLMAugmentationService
        from fastapi import HTTPException
        import utils.constant as const
