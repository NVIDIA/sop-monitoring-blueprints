...
def create_inference_pipeline(
    ...
    **kwargs,
    ...
    mux_width = kwargs.get("mux_width", PREPROCESS_WIDTH)
    mux_height = kwargs.get("mux_height", PREPROCESS_HEIGHT)
    is_camera = False
    ...
    if is_camera:
        os.environ["PYLON_CAMEMU"] = "1"  # enable camera emulation
        pipeline.add("pylonsrc", "pylonsrc", {"device-serial-number": camera_serial_number, "capture-error": 1})
...
def ds_boundary_infernce(
    file_path, score_queue: Queue, threshold: float = 0.8, gpu_id: int = 0, frame_queue: Queue = None
):
    start_time = time.time()
    pipeline = create_inference_pipeline(file_path, score_queue, gpu_id, frame_queue=frame_queue)
...
if __name__ == "__main__":
    ...
    frame_queue = Queue()
    # frame_queue = None
    ds_boundary_infernce(video_path, score_queue, args.threshold, args.gpu_id, frame_queue=frame_queue)
