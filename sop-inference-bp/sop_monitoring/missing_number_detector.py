...
class MissingNumberDetector:
    ...
    def process_number(self, action_number: int) -> dict:
        ...
        try:
            ...
            _LOGGER.error("Action number %d not found in index_to_action_number. "
                          "Maybe VLM output and action.json doesn't match. "
                          "Please check if you are using the correct VLM weight and action.json.", action_number)
            return {}
        ...
        if len(self.seen_in_cycle) > 0:
            ...
            elif (number <= self.N * self.cycle_boundary_threshold_low
            ...
            if is_new_cycle:
                ...
                result['missing_detected'] = [
                    self.index_to_action_number[missing] for missing in missing_in_previous]
                result['misordered_detected'] = [
                    self.index_to_action_number[misordered] for misordered in misordered_in_previous]
                if self.verbose:
                    _LOGGER.debug("Cycle %d completed: missing %s, misordered %s", self.current_cycle - 1, missing_in_previous, misordered_in_previous)
    ...
    def _complete_cycle(self) -> Tuple[List[int], List[int]]:
        ...
        misordered_in_cycle = [m['number'] for m in self.misordered_numbers
                              if m['cycle'] == self.current_cycle]
        ...
        if self.verbose:
            _LOGGER.debug("Cycle %d completed. Seen: %s, Missing: %s, Misordered: %s", self.current_cycle, sorted(self.seen_in_cycle), missing_in_cycle, misordered_in_cycle)
    ...
    def get_statistics(self) -> dict:
        ...
        current_misordered = [m['number'] for m in self.misordered_numbers
                             if m['cycle'] == self.current_cycle]
    ...
    def get_current_cycle_status(self) -> dict:
        ...
        misordered = [m['number'] for m in self.misordered_numbers
                     if m['cycle'] == self.current_cycle]
    ...
    def print_summary(self) -> dict:
        ...
        temp_detector = MissingNumberDetector(N=self.N,
        ...
        for i, num in enumerate(sequence):
            ...
            if not result:
                summary[_cycle_analysis_key].append(f"Error: {num} not a valid action, please check if VLM output and action.json match.")
                continue
        ...
        for cycle_detail in self.cycle_details:
            ...
            if output_parts:
                ...
                summary[_cycle_analysis_key].append(f"Cycle {cycle_num}: {seen_nums_mapped} -> {', '.join(output_parts)}")
            else:
        ...
        if len(self.seen_in_cycle) > 0:
            ...
            if output_parts:
                _LOGGER.info("  Cycle %d: %s -> %s (incomplete)", int(self.current_cycle), current_seen_mapped, ', '.join(output_parts))
                summary[_cycle_analysis_key].append(f"Cycle {self.current_cycle}: {current_seen_mapped} -> {', '.join(output_parts)} (incomplete)")
            else:
                _LOGGER.info("  Cycle %d: %s -> no issues (incomplete)", int(self.current_cycle), current_seen_mapped)
                summary[_cycle_analysis_key].append(f"Cycle {self.current_cycle}: {current_seen_mapped} -> no issues (incomplete)")
