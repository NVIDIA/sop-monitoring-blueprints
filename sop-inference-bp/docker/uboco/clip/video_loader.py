...
class BetterVideoLoader(Dataset):
    """Pytorch video loader."""
    ...
    def _get_video_info(self, video_path):
        ...
        height = int(video_stream['height'])
        # fps = math.floor(convert_to_float(video_stream['avg_frame_rate']))
        fps = convert_to_float(video_stream['avg_frame_rate'])
        try:
            frames_length = int(video_stream['nb_frames'])
            duration = float(video_stream['duration'])
        except Exception:
    ...
    def __getitem__(self, idx):
        ...
        if load_flag:
            ...
            try:
                info = self._get_video_info(video_path)
                # print("info for " + video_path)
                # print(info)
                h, w, avg_fps = info["height"], info["width"], info['fps']
            except Exception:
                print('ffprobe failed at: {}'.format(video_path))
                return {'video': th.zeros(1), 'input': video_path,
                        'output': output_file, 'info': {}}
            ...
            cmd = (
                ffmpeg
                .input(video_path)
                .trim(start_frame=round(self.start_time * avg_fps) )
                .filter('fps', fps=fps)
                .filter('scale', width, height)
            )
            if self.centercrop:
